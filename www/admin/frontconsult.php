...
if (isset($_POST['qr_data'])) {
    if ($connection->connect_error) {
        // If connection failed earlier, return error for AJAX
    ...
    $stmt->close();
    $connection->close(); // Close connection after AJAX response
...
// PHP will now correctly set the date based on Asia/Manila timezone
$date = $_POST['date'] ?? date('Y-m-d');
$timeIn = $_POST['time_in'] ?? (new DateTime('now', new DateTimeZone('Asia/Manila')))->format('H:i'); // Set default TimeIn to Asia/Manila
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_consultation'])) {
    ...
    try {
        ...
        if ($stmt_consultation->affected_rows > 0) {
            ...
            // Redirect to medilog.php with the current date
            echo "<script>alert('Consultation saved successfully and medicine dispensed!'); window.location.href='medilog.php?date=" . urlencode($date) . "';</script>";
...
<body>
    <div class="sidebar" id="mySidebar">
        ...
        <a href="frontconsult.php" class="active">Consultation</a>
    ...
    <script>
        ...
        async function fetchStudentData(qrData) {
            ...
            try {
                const response = await fetch('frontconsult.php', { // AJAX request to the same page
                    method: 'POST',
        ...
        function populateConsultationForm() {
            ...
            // Only set time_in if it's currently empty
            ...
            // Date field will be set by PHP, do not override with JS
            // elements.dateField.value = formatDate(today); // REMOVED: This line caused the yesterday issue
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // PHP now handles setting the initial date correctly.
            // Removed the following block as it caused timezone issues:
            /*
            if (!dateField.value) {
