...
// Include the QR Code library using an absolute path
// __DIR__ resolves to the directory of the current script
require_once __DIR__ . DIRECTORY_SEPARATOR . 'phpqrcode' . DIRECTORY_SEPARATOR . 'qrlib.php';
...
<body>
    <div class="sidebar" id="mySidebar">
        ...
        <a href="generate_qr_codes.php" class="active">Generate QR Codes</a>
    ...
    <div class="main-content" id="mainContent">
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <form action="generate_qr_pdf.php" method="post" target="_blank">
        ...
        <?php if (!empty($patients_by_program)): ?>
            <?php
            // Define the absolute path for the QR codes directory
            $qrCodeDirectory = __DIR__ . DIRECTORY_SEPARATOR . 'qrcodes';
            ...
            if (!file_exists($qrCodeDirectory)) {
                ...
                if (!mkdir($qrCodeDirectory, 0777, true)) {
                    echo '<div class="alert alert-danger" role="alert">Error: Failed to create QR codes directory. Please check folder permissions for "' . htmlspecialchars($qrCodeDirectory) . '".</div>';
                    // Stop further processing if directory cannot be created
                    $patients_by_program = []; // Clear patients to prevent errors in loop
                }
            } elseif (!is_writable($qrCodeDirectory)) {
                 echo '<div class="alert alert-danger" role="alert">Warning: QR codes directory is not writable. Please check folder permissions for "' . htmlspecialchars($qrCodeDirectory) . '". QR codes might not be generated.</div>';
            }
            ?>
                            ...
                            <table class="table student-table" id="table-<?php echo md5($program); ?>">
                                ...
                                <tbody>
                                    ...
                                    <?php foreach ($patients as $patient): ?>
                                        ...
                                        // Construct the full absolute path for the QR code file
                                        $qrFileName = $qrCodeDirectory . DIRECTORY_SEPARATOR . 'patient_' . $patient['PatientID'] . '.png';
                                        ...
                                        if (!file_exists($qrFileName)) {
                                            try {
                                                ...
                                                // Log the error and optionally show a message
                                                error_log("QR Code generation failed for PatientID " . $patient['PatientID'] . ": " . $e->getMessage());
                                                // You might want to display a placeholder or a broken image icon here
                                                $qrFileName = 'images/qr_placeholder.png'; // Fallback image
                                                echo '<script>console.error("QR Code generation failed for ' . htmlspecialchars($patient['Student_Num']) . ': ' . htmlspecialchars($e->getMessage()) . '");</script>';
                                            }
                                        }
                                        ...
                                        if (!$update_stmt->execute()) {
                                            error_log("Database update failed for PatientID " . $patient['PatientID'] . ": " . $update_stmt->error);
                                        }
                                        ?>
                                        <tr data-full-name="<?php echo htmlspecialchars($patient['FirstName'] . ' ' . $patient['LastName']); ?>" data-student-num="<?php echo htmlspecialchars($patient['Student_Num']); ?>">
                                            ...
                                            <td>
                                                <button type="button" class="btn btn-info btn-sm" data-toggle="modal" data-target="#qrCodeModal"
                                                        data-qr-image="<?php echo htmlspecialchars(str_replace(__DIR__ . DIRECTORY_SEPARATOR, '', $qrFileName)); ?>"
