                        ...
                        if ($recordsResult->num_rows > 0) {
                            ...
                            foreach ($patientConsultations as $patientID => $consultations) {
                                ...
                                foreach ($consultations as $consultation) {
                                        ...
                                        if (count($consultations) > 1) {
                                            ...
                                            echo "<div class='dropdown'>
                                                    <button class='btn btn-link dropdown-toggle' type='button' data-toggle='dropdown'>
                                                        " . htmlspecialchars($consultation['FullName']) . "
                                                    </button>
                                                    <div class='dropdown-menu'>";
                                            foreach ($consultations as $drop) {
                                                // Note: patient_records.php is not provided, assuming it exists
                                                echo "<a class='dropdown-item' href='staffpatient_records.php?id=" . $drop['PatientID'] . "&consult=" . $drop['ConsultationID'] . "'>" .
                                                    htmlspecialchars($drop['Date']) . " - " . htmlspecialchars($drop['Assessment']) . "</a>";
                                            }
                                            echo "</div></div>";
                                        } else {
                                            // Show regular name (as a link or plain text)
                                            echo "<a href='staffpatient_records.php?id=" . $consultation['PatientID'] . "&consult=" . $consultation['ConsultationID'] . "'>" .
                                    ...
                                    echo "<td data-label='Medicine Given'>" . htmlspecialchars($consultation['MedicineGiven']) . "</td>"; // Display Medicine Given
                                    echo "<td data-label='Quantity Given'>" . htmlspecialchars($consultation['QuantityGiven']) . "</td>"; // Display Quantity Given
